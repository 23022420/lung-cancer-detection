...
After evaluating 3 different classification algorithms the GradientBoostingClassifier had the best accuracy of ~91% and that's the algorithm used to make predictions.
""")
if st.button('Get Prediction'):
    ...
    #res = re.post(f"http://0.0.0.0:8000/predict",json=values)
   # res = re.post("http://127.0.0.1:8000/predict", json=values)
    #res = re.post("http://backend:8000/predict", json=values)
    res = re.post("http://127.0.0.1:8000/predict", json=values)
